...
import json
import configparser
from .exceptions import InvalidConfigFormat
from .exceptions import InvalidConfigFormat
...
class Config:
    ...
    def __validate_format(self, fmt):
        if fmt not in ["json", "ini"]:
            raise InvalidConfigFormat("Invalid config file format, valid formats include 'json' and 'ini'")
        else: self.fmt = fmt
    ...
    def serialize_safe(self):
        ...
        for path in self.paths:
            if os.path.isfile(path):
                good_paths.append(path)
        if len(good_paths) < 1:
            raise NoConfigFileFound("No file found to write to")
        else:
            self.location = good_paths[0]
